...
[逐页动画 V4](projects/development-history-v4-pagewise/script.md)针对本地审片修正了
总览页深色背景上的黑字，重写 18 段口播并以“乖巧萌妹”TTS → suiV2 +4
重新配音；字幕上移以避开页脚和进度线。逐页检查和去模板句式的口播提示词
见[制作指南](docs/visual-explainer-workflow.md)。配音素材与成片保存在本地忽略目录，
源码只保留引用路径。

## 已知缺陷

这套工作流已经能稳定产出可审阅视频，但当前成片仍有三个明确短板：
...
- **网页 PPT 的动效还没有充分承担原理讲解**：已有弹幕、上下文、队列和时间线动画，
  ...
  更连续的可视化过程。
- **V4 旁白的听感仍不理想**：虽已用“乖巧萌妹”TTS 和 suiV2 +4 替换旧稿，
  语气、自然度与节奏仍需人工试听、逐句调整；现有字幕时间按句段长度估算，
  还没有完成词级对齐。

后续制作采用[逐页设计与审片流程](docs/visual-explainer-workflow.md)：先定全片结构，
再为每页单独生成、渲染和审核动画，最后统一旁白、字幕与转场节奏。
